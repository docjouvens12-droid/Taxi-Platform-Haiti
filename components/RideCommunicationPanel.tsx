...
export default function RideCommunicationPanel() {
  ...
  return <>
    ...
    <style jsx>{`
      .rideComActions{position:fixed;left:50%;transform:translateX(-50%);z-index:12070;display:flex;gap:8px;pointer-events:auto}.rideComActions.passenger{bottom:max(28px,calc(env(safe-area-inset-bottom) + 14px))}.rideComActions.driver{bottom:max(18px,calc(env(safe-area-inset-bottom) + 10px))}.rideComActions.cleanDriver{left:auto;right:16px;bottom:56%;transform:none}.rideComActions button{border:0;border-radius:999px;padding:10px 15px;font:850 12px Inter,system-ui,sans-serif;box-shadow:0 9px 24px rgba(16,32,51,.20)}.callBtn{background:#102033;color:#fff}.chatBtn{background:${isDriver || isCleanDriver ? '#0f8067' : '#1b70eb'};color:#fff}
