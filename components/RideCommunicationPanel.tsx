...
export default function RideCommunicationPanel() {
  ...
  const isPassenger = pathname === '/passenger/dashboard' || pathname === '/' || pathname === '/movi'
  const isDriver = pathname === '/driver/dashboard'
  const isCleanDriver = pathname === '/driver/dashboard-v2'
  const enabled = isPassenger || isDriver || isCleanDriver
  ...
  return <>
    <div className={`rideComActions ${isDriver || isCleanDriver ? 'driver' : 'passenger'} ${isCleanDriver ? 'cleanDriver' : ''}`}>
    ...
    <style jsx>{`
      .rideComActions{position:fixed;left:50%;transform:translateX(-50%);z-index:12070;display:flex;gap:8px;pointer-events:auto}.rideComActions.passenger{bottom:max(28px,calc(env(safe-area-inset-bottom) + 14px))}.rideComActions.driver{bottom:max(18px,calc(env(safe-area-inset-bottom) + 10px))}.rideComActions.cleanDriver{left:auto;right:16px;bottom:56%;transform:none}.rideComActions button{border:0;border-radius:999px;padding:10px 15px;font:850 12px Inter,system-ui,sans-serif;box-shadow:0 9px 24px rgba(16,32,51,.20)}.callBtn{background:#fff;color:#102033;border-color:#cfd9e6}.chatBtn{background:${isDriver || isCleanDriver ? '#0f8067' : '#102033'};color:#fff;border-color:#102033}
      .rideComActions button{border:1px solid #dce6f3;border-radius:16px;min-width:128px;height:52px;padding:0 18px;font:800 14px Inter,system-ui,sans-serif;box-shadow:0 10px 24px rgba(16,32,51,.14);display:flex;align-items:center;justify-content:center;gap:7px}
